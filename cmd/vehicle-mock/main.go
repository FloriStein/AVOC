...
func main() {
	...
	go func() {
		for {
			...
			// Minted fresh per attempt — a token signed once at process start
			// expires after 24h and would strand long-running containers in a
			// permanent reconnect-fail loop after their first day of uptime.
