...
| ID | Task | Typ | Status | Abhängigkeiten |
|----|------|-----|--------|-----------------|
| GOSTYLE-IF-01 | `recording.SessionRecorder` (6 Methoden, aktuell nirgends als Interface-Typ konsumiert): klären, ob Interface tatsächlich verwendet werden soll (`*MemoryRecorder` → `SessionRecorder` in `cmd/control-server/main.go:102`) oder aufgelöst wird, solange nur eine Implementierung existiert (Rule 1.2 — Abstraktion ohne Konsument ist unbegründet) | S | 🔄 Sprint 34 | HEX-05 |
| GOSTYLE-IF-02 | `fleetgateway.FleetGateway` (3 Methoden, ungenutzt als Typ): gleiche Frage wie IF-01. `fleetservice.Dispatcher` (1 Methode) ist bereits der korrekte konsumentenseitige Schnitt und bleibt unverändert | S | 🔄 Sprint 34 | HEX-05 |
| GOSTYLE-IF-03 | `pkg/audit.AuditWriter` (3 Methoden) in schlankes `WriteSync`-only Interface für die 5 Safety-/Command-Consumer aufspalten; `Close`/`QueryBySession` bleiben am konkreten Typ bzw. eigenem kleineren Interface für `main.go` | M | 🔲 Backlog | HEX-05 — zurückgestellt auf Sprint 35 (Token-Budget-Grill-Me 2026-07-18) |
| GOSTYLE-IF-04 | `authservice.UserStore` (7 Methoden): `SeedAdmin` (reine Bootstrap-Methode, bereits am konkreten Typ genutzt) aus dem Interface entfernen; verbleibende 6 Methoden gegen tatsächlichen `Handler`-Bedarf prüfen | M | 🔲 Backlog | HEX-05 — zurückgestellt auf Sprint 35 (Token-Budget-Grill-Me 2026-07-18) |
| GOSTYLE-IF-05 | `vehicleregistry.VehicleStore` (5 Methoden): `SeedDefault` (Bootstrap) aus Interface lösen, analog IF-04 | S | 🔄 Sprint 34 | HEX-05 |
| GOSTYLE-IF-06 | `controlserver/safety.Publisher` (2 Methoden) in `PublishEvent`-only Interface für `Engine`/Watchdogs aufteilen; `TriggerEmergencyStop` bleibt eigener Zugriffspfad, analog zum bereits vorbildlichen `vehicleconnection.safetyPublisher`-Muster | S | 🔄 Sprint 34 | HEX-05 |
...
| ID | Task | Typ | Status | Notizen |
|----|------|-----|--------|---------|
| SEC-01 | JWT-Alg-Confusion-Check fehlt an 3 von 5 `jwt.Parse*`-Stellen | S/M | 🔄 Sprint 34 | Nachrecherchiert 2026-07-18 (Sprint-34-Planung) — tatsächlich 3 von 5, nicht 2 von 4 wie beim GOSTYLE-Duplikat-Scan vermerkt. **Fehlt:** `internal/authservice/handler.go:320` (`parseToken`), `internal/controlserver/transport/websocket.go:311` (`validateJWT`), `internal/vehicleconnection/handler.go:165` (`validateJWT`) — alle drei nutzen `jwt.ParseWithClaims` ohne Prüfung von `t.Method`, akzeptieren also jeden vom Client behaupteten Signatur-Algorithmus (inkl. `none`/asymmetrisch), nicht nur HMAC mit dem Server-Secret. **Bereits korrekt:** `internal/fleetservice/handler.go:63`, `cmd/control-server/main.go:889` (beide prüfen `t.Method.(*jwt.SigningMethodHMAC)`). Fix: identische Prüfung in den drei Callback-Funktionen ergänzen. Braucht laut CLAUDE.MD Abschnitt 17 Testabdeckung für Fehlerpfade (falscher Algorithmus, `alg: none`, kaputtes Secret) an allen drei Stellen, nicht nur Happy-Path. |
