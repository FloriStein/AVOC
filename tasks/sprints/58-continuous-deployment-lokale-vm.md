...
**Abgeschlossen 2026-07-22.** Alle 6 Tasks umgesetzt und real verifiziert:

- **CD-01:** Self-hosted Runner (`avoc-dev-runner`, Label `avoc-local-deploy`) auf dem Dev-Rechner
  registriert, als systemd-Service eingerichtet, Scope auf dieses Repo beschränkt. Vorher verifiziert:
  "Require approval for all outside collaborators" ist aktiv.
- **CD-02..04:** Neuer Workflow `.github/workflows/deploy-local-vm.yml` — Trigger `workflow_run`
  nach `docker-build.yml`, hart gefiltert auf `event=push`/`head_branch=main`/`conclusion=success`
  (nie `pull_request`), `concurrency`-Gruppe `deploy-local-vm`. GHCR-Login über das ephemere
  `secrets.GITHUB_TOKEN` (kein persönlicher PAT im CD-Pfad). Post-Deploy-Health-Check gegen
  Frontend-HTTPS + Control-Server `/health`, kein automatischer Rollback. Runner-lokale, nicht
  committete Inventory-Override `~/avoc-deploy/hosts.local.ini` löst das Problem, dass die echte
  VM-IP nie im Repo steht.
- **CD-05:** Real verifiziert — nach dem PR-#9-Merge (971bb2e→7ac7b56) lief `docker-build.yml` auf
  `main`, danach sprang `deploy-local-vm.yml` automatisch an (Run 29911617894), alle Schritte
  inkl. Health-Check grün, ganz ohne manuellen `ansible-playbook`-Aufruf — der volle CD-Pfad
  funktioniert end-to-end.
- **CD-06:** README.md (neuer Abschnitt "CI-Image-Publishing & Continuous Deployment"),
  `docs/deployment/UEBERGABE-ABWEICHUNGEN.md` (Ergänzung zu Abweichung 2), `DECISIONS.MD`
  (neue Zeile), `tasks/backlog.md` aktualisiert.

**Nebenergebnis (nicht ursprünglich geplant, aus derselben Sitzung):** Nutzer bat direkt im
Anschluss um Planung eines Folge-Sprints für die seit Sprint 55 informational mitlaufenden, aber
nie behobenen `gosec`/`npm audit`-CI-Findings — als neues EPIC "CI-Sicherheits-Findings beheben"
(Sprint 59, SEC-CI-01..06) in `tasks/backlog.md` geplant, noch nicht umgesetzt.
