...
Merge manuell angestoßen werden. Ziel: `push nach main → CI baut+pusht Images (Sprint 56) →
automatischer Deploy auf avoc-local-vm`, ohne manuellen Schritt.

**Kritische Randbedingung:** Das AVOC-Repo ist **public**. GitHub-gehostete Runner haben keinen
Netzwerkzugriff auf `avoc-local-vm` (kein öffentliches IP, bewusst keine Portweiterleitung, s.
`ansible/roles/firewall`) — ein Deploy-Schritt muss also auf einem **Self-hosted Runner** auf dem
Dev-Rechner laufen. Self-hosted Runner + public Repo ist ein bekanntes Sicherheitsrisiko (beliebige
PR-Autoren könnten sonst Code auf dem Runner — und damit potenziell gegen `avoc-local-vm` —
ausführen). Zwingende Mitigation, nicht optional:
- Der Deploy-Job läuft **ausschließlich** auf `push`-Events nach `main` (niemals auf
  `pull_request`, auch nicht `pull_request_target`) — durch Branch-Protection (CIPUB-04) kann dort
  nur landen, was den Nutzer-Merge durchlaufen hat.
- Repo-Setting "Require approval for all outside collaborators" (GitHub → Settings → Actions →
  Fork pull request workflows) muss aktiv sein/bleiben, bevor der Runner registriert wird.
- Der Deploy-Job bleibt ein **separater Job/Workflow**, getrennt von den GitHub-gehosteten
  Build-/Test-Jobs, die weiterhin auch auf `pull_request` laufen — keine Vermischung von
  Runner-Typen in einem Job.

**Nutzerentscheidungen (Rückfrage 2026-07-22):**
- **CD-Ziel:** Nur die lokale Test-VM (`avoc-local-vm`). Kein echter Hetzner-Server in diesem
  Sprint — der existiert noch nicht und wäre ein eigenes, größeres Vorhaben (Kosten, DNS, TLS).
- **Mechanismus:** Self-hosted GitHub Actions Runner auf dem Dev-Rechner (native
  Actions-Integration, Logs/Historie in der GitHub-UI) statt eines von GitHub Actions losgelösten
  lokalen Poll-/Cron-Skripts.
- **Trigger:** Automatisches Deployment bei jedem Push nach `main` (echtes Continuous Deployment,
  kein manuelles Freigabe-Gate über GitHub Environments — angemessen für eine Test-/Staging-VM).
...
**Sprint-Zuweisung (2026-07-22, Nachbesprechung Sprint 59):** Nutzerwunsch, alle verbliebenen
`DRIFT-K*`/`DRIFT-M18..M25`-Restposten einzuplanen. Zu groß für einen Sprint (Token-Budget,
zwei Grill-Me-Entscheidungen + mehrere M-Tasks) — aufgeteilt: **Sprint 60** (`DRIFT-K4/K5/K6`,
...
| DRIFT-K4 | „CI Build-Fail" bei >100ms existiert nicht als Pipeline — Grill-Me vor Fix | L | Kritisch | ✅ behoben (Sprint 60, 2026-07-23) | Prämisse war überholt (`test-latency.yml` existiert seit Sprint 41), lief aber non-blocking. Grill-Me-Entscheidung `DRIFT60-02`: CI-Historie geprüft (47 Läufe, p99 durchgängig 0-1ms bei 100ms-Budget, nie Rausch-bedingt gescheitert) — `go-benchmark`-Job jetzt blocking (`continue-on-error` entfernt), ADR-006 Update-Block |
| DRIFT-K5 | WS-ACK-Roundtrip-Benchmark läuft im Standard-Testlauf nicht mit — Grill-Me vor Fix | M/L | Kritisch | ✅ Sprint 55 | Real bereits durch `CIHARD-01` (Sprint 55) behoben, `make test-latency` läuft seitdem grün ohne Skip — diese Zeile war nur nie nachgezogen (Backlog-Pflegefehler). Status-Korrektur `DRIFT60-01`, Sprint 60 |
| DRIFT-K6 | `TestLatencyBudget_DocumentedRequirement` ist tautologisch — Grill-Me vor Fix | S/M | Kritisch | ✅ behoben (Sprint 60, 2026-07-23) | War weiterhin unbehoben (`CIHARD-01`s Behauptung "schließt DRIFT-K5/K6 endgültig ab" stimmte nur für K5). Grill-Me-Entscheidung `DRIFT60-03`: Test ersatzlos gestrichen — `BenchmarkControlACKRoundtrip`s eigener `b.Fatalf` bei p99>100ms (ADR-010) verifiziert die Anforderung bereits, der separate Test prüfte nur, dass die Konstante sich selbst nicht geändert hat |
...
| DRIFT-M19 | k6-Latenztest auf echten WS-ACK-Roundtrip umstellen (misst aktuell `GET /state`) | M | Mittel | ✅ behoben (Sprint 61, `DRIFT61-01`) | `tests/performance/latency.js` nutzt jetzt `k6/ws` (`ws.connect`, binäre `DEADMAN_HOLD`-Nachricht wie `BenchmarkControlACKRoundtrip`) statt `GET /sessions` als HTTP-Proxy; `vus` bewusst 1 (ein `ACTIVE_OPERATOR`-Slot pro Fahrzeug, konkurrierende VUs würden nur Lock-Contention messen). `make test-k6` grün, p99=1ms |
| DRIFT-M20 | Video-Latenzziel (100–300ms) — automatisierten Test ergänzen oder Doku als unverifiziert kennzeichnen | S/M | Mittel | ✅ behoben (Sprint 61, `DRIFT61-02`) | Entscheidung: Doku statt Test — `docs/requirements.md`/`CONTEXT.MD` markieren das Ziel jetzt explizit als "unverifiziert/aspirational" (ADR-014: kein Safety-Hartziel, ein `getStats()`-Mock würde nur Mock-Lesecode beweisen, ein echter E2E-Test bräuchte unverhältnismäßige MediaMTX+Browser-Infrastruktur) |
| DRIFT-M21 | `docs/deployment/ec2-bootstrap.md` um `fleet-service` ergänzen | S | Mittel | ✅ behoben (Sprint 61, `DRIFT61-03`) | Reiner Doku-Fix (3 Stellen, nicht nur die vorrecherchierten 2) — `fleet-service` ist seit Sprint 36 (`DEPLOY-08`) längst in `docker-compose.prod.yml`/nginx verdrahtet, braucht keinen eigenen Security-Group-Port (analog `auth-service`) |
| DRIFT-M22 | `-race` strukturell in Makefile-Standardziel(en) verankern statt nur ad-hoc | M | Mittel | ✅ behoben (Sprint 61, `DRIFT61-04`) | Neues eigenständiges `test-race`-Target (nicht in `test-unit` gebacken, um dessen schnellen Dev-Loop nicht zu verlangsamen) + neuer blockierender `race`-CI-Job. Voller `-race`-Lauf gegen den Gesamtcode fand keine bis dahin unentdeckten Races |
...
| DRIFT-M24 | `OperatorRole` als Typ statt Rohstring durchgängig verwenden (`session/manager.go` u. a.) | M | Mittel | ✅ behoben (Sprint 61, `DRIFT61-05`) | Package-lokaler `session.OperatorRole`-Typ (`RoleActiveOperator`/`RoleObserver`), kein Import von `authservice.OperatorRole` (separate Services). Alle Rohstring-Stellen in `session/manager.go`/`handover.go`/`command/engine.go`/`transport/websocket.go`/`cmd/control-server/main.go` ersetzt, keine Verhaltensänderung, `-race -count=2` grün. Gemini-Zweitmeinung fand dabei einen echten, vorbestehenden Nebenbefund (`PushSFUEvent`-Data-Race, dokumentiert in `CONTEXT.MD`, bewusst nicht mitgefixt) |
| DRIFT-M25 | Task-Status in `internal/fleetservice/store.go` als benannten Typ statt Rohstrings führen | M | Mittel | ✅ behoben (Sprint 61, `DRIFT61-06`) | Package-lokaler `fleetservice.TaskStatus`-Typ (Vorbild `statemachine/state.go`), `Task.Status`/`taskTransitionSources`/`allowedTaskTransitions` umgestellt; `UpdateTaskStatus`s Parameter bleibt bewusst `string` (roher HTTP-PATCH-Body, Validierung am Map-Lookup). Keine Verhaltensänderung, `-race -count=2` grün, echte Postgres-Integrationstests bestätigen `pq.Array`/`Scan` funktionieren mit dem neuen Typ |
...
| ID | Task | Typ | Abhängigkeiten |
|----|------|-----|-----------------|
| CIHARD-01 | `BenchmarkControlACKRoundtrip`-Skip-Bug beheben (`vehicle_id` korrigieren + `session_id`-Query-Parameter ergänzen) — schließt DRIFT-K5 endgültig ab (DRIFT-K6 blieb offen, s. `tasks/current-sprint.md` `DRIFT60-03`). | S | ✅ Sprint 55 |
