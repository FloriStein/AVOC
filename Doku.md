# AVOC auf einer eigenen EC2-Instanz zum Laufen bringen

Diese Anleitung richtet sich an jemanden, der dieses Repository frisch klont und
den kompletten Stack (Frontend, Control Server, Auth/Safety/Telemetry-Services,
MediaMTX WHIP/WHEP, coturn, PostgreSQL, MQTT, Grafana/Loki) auf einer **eigenen**
AWS-EC2-Instanz zum Laufen bringen will.

Sie beschreibt den tatsächlich funktionierenden Weg (validiert am 2026-07-09
gegen eine echte laufende Instanz), nicht nur den in `docs/deployment/ec2-bootstrap.md`
ursprünglich geplanten. An den Stellen, wo beide Dokumente auseinanderlaufen, wird
das unten explizit benannt — siehe [Abweichung: keine Docker-Hub-Registry](#abweichung-keine-docker-hub-registry).

**Architekturhintergrund:** [README.md](README.md), [docs/architecture.md](docs/architecture.md),
[docs/adr/](docs/adr/). Diese Datei behandelt nur das *Deployment*, keine Architekturentscheidungen.
...
## Schritt 2 — Secrets im SSM Parameter Store anlegen

Einmalig vom Dev-Rechner:
...
(JWT-Secret, WHIP-Stream-Key, TURN-Credentials, Grafana-Login, DB-Passwort,
Admin-Passwort, Docker-Hub-Credentials).

### Abweichung: keine Docker-Hub-Registry

`setup-ssm.sh` fragt zusätzlich nach `DB_PASSWORD`, `ADMIN_PASSWORD`,
`DOCKER_USERNAME` und `DOCKER_PASSWORD` und legt sie in SSM ab — `scripts/deploy.sh`
liest diese vier Parameter aber **nicht** aus SSM:

- `DB_PASSWORD` und `ADMIN_PASSWORD` müssen stattdessen in einer lokalen
  `.env`-Datei **auf der Instanz** liegen (`docker compose` liest sie automatisch
  aus dem Arbeitsverzeichnis) — siehe Schritt 4.
- `DOCKER_USERNAME`/`DOCKER_PASSWORD` werden nur für den ursprünglich geplanten
  `make push`-Weg (Bauen → Docker Hub → `docker compose pull` auf der Instanz)
  gebraucht. Diese Anleitung nutzt stattdessen **lokal bauen → `docker save` →
  `scp` → `docker load`** (siehe `docker-compose.prod.yml`-Kopfkommentar:
  "Übergabe-Abweichung von ADR-019 — kein Docker Hub"). Das spart eine Docker-Hub-
  Registrierung; wer lieber über eine Registry deployen will, kann stattdessen
  `make push` (siehe [Makefile](Makefile)) verwenden und `deploy.sh` entsprechend
  um einen `docker compose pull` ergänzen.
...
## Schritt 3 — Verzeichnisstruktur + Config-Dateien auf die Instanz kopieren
...
├── deploy.sh
├── .env                              # Schritt 4 — DB_PASSWORD, ADMIN_PASSWORD
...
## Schritt 4 — `.env` auf der Instanz anlegen
...
## Schritt 5 — Images lokal bauen und auf die Instanz übertragen

Alle 7 Images für `linux/amd64` bauen (Docker Desktop muss laufen):
...
1. Holt `JWT_SECRET`, `WHIP_STREAM_KEY`, `TURN_EXTERNAL_IP`, `TURN_REALM`, `TURN_USER`,
   ...
   (`DB_PASSWORD`/`ADMIN_PASSWORD` kommen aus `.env`, siehe Schritt 4)
...
Sechs Go-Binaries unter `cmd/`, jedes ein eigenes `main.go` plus die
...
Zweifel immer alle 7 Images neu zu bauen (siehe Schritt 5), statt selbst
nachzuverfolgen, was betroffen ist.

**4. Auf die Instanz bringen und ausrollen** — Schritt 5 + 6 dieser Anleitung
bzw. verkürzt:
```bash
...
```bash
# 1. Alle 7 Images neu bauen (siehe Schritt 5) — Build-Cache macht das schnell
# 2. docker save + scp + docker load (siehe Schritt 5)
# 3. Bei Config-Änderungen (docker-compose.prod.yml, mediamtx.yml, ...): erneut scp'en
# 4. deploy.sh erneut ausführen (siehe Schritt 6) — recreated nur geänderte Container
```
...
falls mit versionierten Tags statt `latest` gearbeitet wird.
...
```
Zusätzlich: Security Group muss Port 3478 (TCP+UDP) und UDP 49152–65535 offen haben.
...
- [docs/deployment/ec2-bootstrap.md](docs/deployment/ec2-bootstrap.md) — ursprünglicher, Docker-Hub-basierter Deployment-Plan
- [docs/deployment/hetzner-setup.md](docs/deployment/hetzner-setup.md) — alternative Deployment-Zielumgebung
...
- [docs/adr/](docs/adr/) — Architekturentscheidungen (ADR-014 TURN, ADR-019 Deployment, ADR-020 MediaMTX, ADR-023 PostgreSQL)
