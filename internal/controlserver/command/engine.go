...
// State Machine and Watchdogs are looked up per-vehicle via the Registry on
// every Handle() call (ADR-026) — there is no single injected sm/deadman
// anymore, since two vehicles must never share safety state.
type Engine struct {
...
func (e *Engine) Handle(rawMsg []byte, sess session.Session) ([]byte, error) {
	...
	vc := e.vehicleContexts.Get(sess.VehicleID)
