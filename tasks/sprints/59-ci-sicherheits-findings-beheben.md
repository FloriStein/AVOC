...
Datum: 2026-07-22 | Status: ✅ abgeschlossen.
...
**Abgeschlossen 2026-07-22.** Alle 6 Tasks umgesetzt und real gegen einen lokal installierten
`gosec` (nicht nur den PR-#9-Snapshot) verifiziert:

- **SEC-CI-01 (`G104`, 32 Findings):** Alle `json.NewEncoder(w).Encode(...)`-Aufrufe (20 Stellen
  in `control-server`, `safety-service`, `telemetry-service`, `webrtc-sfu`, `authservice`,
  `fleetservice`) werfen den Fehler jetzt in ein `if err := ...; err != nil { log.Warn(...) }`.
  Die restlichen 12 Findings waren beim Sprint-Start nicht im Plan-Snapshot enthalten (der hatte
  nur Encode-Aufrufe erfasst) — echte weitere G104-Treffer: `Close()`-Aufrufe auf DB-Writer/HTTP-
  Response-Bodies/WS-/WebRTC-Peer-Connections (strukturiert geloggt) sowie zwei
  `conn.SetReadDeadline(...)` in WebSocket-`PongHandler`n, die den Fehler jetzt direkt
  zurückgeben statt `nil` zu erzwingen (echte Korrektur, kein reines Silencing).
- **SEC-CI-02 (`G114`, 6 Findings):** Alle 6 `cmd/*/main.go` nutzen jetzt `http.Server{Addr:,
  Handler:, ReadHeaderTimeout: 10s, ReadTimeout: 10s, WriteTimeout: 30s}` statt nacktem
  `http.ListenAndServe`. Werte + Begründung als neue Pflichtregel in `docs/go-style-guide.md`
  dokumentiert; WebSocket-Verbindungen (Hijack durch `gorilla/websocket`) sind von den
  Server-Timeouts nicht betroffen.
- **SEC-CI-03 (`G304`, 1 Finding):** `pkg/mqtttls/mqtttls.go` — `caCertPath` ist ein
  Operator-Config-Wert (CA-Zertifikatspfad für MQTTS), nie aus User-/Netzwerk-Input abgeleitet,
  aktuell noch ohne Aufrufer verdrahtet (vorbereitete Infrastruktur). Begründetes False-Positive,
  mit `#nosec G304 --`-Kommentar markiert.
- **Zusätzlich beim lokalen Lauf entdeckt (nicht im ursprünglichen Snapshot):** 5× `G404`
  (schwacher Zufallszahlengenerator) in reinem Simulationscode (`fleetgateway/mock.go`,
  `vehicle-mock/fleet_simulator.go`) — kosmetischer Bewegungs-/Alarm-Jitter für die Fahrzeug-
  Simulation, keine Security-Relevanz, ebenfalls per `#nosec G404 --` markiert.
- **SEC-CI-04 (`npm audit`, 4 Vulnerabilities):** 3 von 4 durch `npm audit fix` automatisch
  behoben (`brace-expansion`, `js-yaml`, `undici`). `esbuild` (Dev-Server-only, Windows,
  transitiv über `vite@7.3.5`) ließ sich nicht automatisch fixen, da `vite` selbst noch die
  verwundbare Version deklariert — gelöst über einen gezielten `overrides`-Eintrag
  (`esbuild: ^0.28.1`) in `frontend/package.json` statt eines riskanten `vite`-Major-Bumps.
  Danach `npm audit`: 0 vulnerabilities; `npm run build` und die volle Vitest-Suite (370 Tests,
  34 Dateien) grün.
- **SEC-CI-05:** `continue-on-error: true` aus beiden Jobs in `security-scan.yml` entfernt.
  Branch-Protection für `main` (nach expliziter Nutzerbestätigung, MB-Regel für risikoreiche/
  schwer umkehrbare Aktionen) um `gosec (Go)` und `npm audit (Frontend)` erweitert — jetzt 6
  Required Status Checks statt 4 (Ergänzung zu CIPUB-04/Sprint 56).
- **SEC-CI-06:** `DECISIONS.MD` (neue Zeile), `docs/go-style-guide.md` (neuer Abschnitt
  HTTP-Server-Timeouts), `README.md` ("Required Status Checks" aktualisiert),
  `tasks/backlog.md` aktualisiert.

**Lokale Verifikation:** `gosec ./...` 44 → 0 Findings; `go vet ./...` sauber; `go test` (alle
Pakete außer `tests/integration`, das einen laufenden Docker-Stack braucht) grün; `npm audit`
4 → 0 Vulnerabilities; `npm run build` + `npm test` (Frontend) grün.
