...
| DRIFT-K4 | „CI Build-Fail" bei >100ms existiert nicht als Pipeline — Grill-Me vor Fix | L | Kritisch | 🔄 Sprint 60 geplant | Bei Sprint-60-Vorrecherche gegen echten Code gegengeprüft: Prämisse überholt, `test-latency.yml` existiert seit Sprint 41, läuft aber bewusst non-blocking (ADR-006-Abweichung) — offene Frage ist Beibehalten vs. jetzt auf blocking umstellen. Fix/Entscheidung + Status-Update dieser Zeile ist `DRIFT60-02` in `tasks/current-sprint.md` |
| DRIFT-K5 | WS-ACK-Roundtrip-Benchmark läuft im Standard-Testlauf nicht mit — Grill-Me vor Fix | M/L | Kritisch | 🔄 Sprint 60 geplant | Bei Sprint-60-Vorrecherche gegen echten Code gegengeprüft: real bereits durch `CIHARD-01` (Sprint 55) behoben, `make test-latency` läuft seitdem 2× hintereinander grün ohne Skip — diese Zeile war nur nie nachgezogen. Reine Status-Korrektur ist `DRIFT60-01` in `tasks/current-sprint.md` |
| DRIFT-K6 | `TestLatencyBudget_DocumentedRequirement` ist tautologisch — Grill-Me vor Fix | S/M | Kritisch | 🔄 Sprint 60 geplant | Bei Sprint-60-Vorrecherche gegen echten Code gegengeprüft: weiterhin unbehoben, `CIHARD-01`s Behauptung "schließt DRIFT-K5/K6 endgültig ab" stimmte nur für K5. Fix/Entscheidung ist `DRIFT60-03` in `tasks/current-sprint.md` |
...
| DRIFT-M18 | OBSERVATION-Trigger „Auth-Service-down blockiert neue Sessions" — implementieren oder Doku korrigieren | M | Mittel | 🔄 Sprint 60 geplant | Bei Sprint-60-Vorrecherche gegen echten Code gegengeprüft: `CONTEXT.MD` behauptet einen Produktivpfad, `grep` über `internal/`/`cmd/` findet keinen — reine Doku-Behauptung. Entscheidung ist `DRIFT60-04` in `tasks/current-sprint.md` (volle Watchdog-Implementierung, falls so entschieden, wäre eigener Folge-Task, nicht mehr Teil von Sprint 60) |
